    ...
    node (label) {
       ...
       stage ('Docker Build'){
          container('kaniko') {
                stage('Build Image') {
                    sh '''
                      /kaniko/executor --context `pwd` --destination qwerty703/eos-gateway-api:latest
                    '''
                }
            }
        }
        ...
        stage ('Helm Chart') {
          container('build') {
            dir('charts') {
              ...
              sh '/usr/local/bin/helm push-artifactory gateway-api-1.0.tgz https://eosadmin.jfrog.io/artifactory/eos-helm-helm-local --username $username --password $password'
              }
            }
        }
        }
